...
\newcommand{\naslovdela}{Numerično reševanje elastostatičnih problemov z brezmrežnimi metodami}
...
\url{http://www.ams.org/msc/msc2010.html?t=65Mxx&btn=Current}
\vfill\noindent
{\bf Math. Subj. Class. (2010):} 74B05, 65N99, 65Y20 \\[1mm]
{\bf Ključne besede:} linearna elastostatika, brezmrežne metode \\[1mm]
{\bf Keywords:} linear elastostatics, meshless methods
...
močno uporablja v strojništvu in gradbeništvu-QUOTES. Enačbe, ki podajajo količine, ki nas zanimajo,
so zelo redko rešljive na roke ali analitično in ne preostane nam drugega, kot da se reševanja lotimo z
računalnikom. Numerično se probleme linearne elastičnosti ponavadi rešuje v šibki obliki z metodo
končnih elementov-QUOTE. Toda že v preteklosti so probleme reševali v močni obliki z metodo končnih
diferenc-QUOTE in metodo končnih volumnov-QUOTE. Poleg tega so se v bolj bližji preteklosti za
reševanje elastostatičnih problemov uporabljati tudi novejše brezmrežne metode v močni in šibki
obliki-QUOTE. Prednost brezmrežnih metod je, da ne potrebujejo polne diskretizacije domene (npr.\ z
večkotniki) kot jo metoda končnih elementov, temveč le točke v domeni in poznavanje njihove lokalne
medsebojne lege, kar omogoča večjo splošnost, lažjo posplošitev na višje dimenzije-QUOTE in
elegantnejšo implementacijo. Poleg tega se sumi, da imajo prednost pri reševanju nekaterih problemov, kot
npr.\ pri problemih, kjer je treba premikati rob domene ali točke v notranjosti, kot na primer
simulacije širjenja razpok-QUOTE. Eno izmed brezmrežnih metod s sodelavci razvijamo tudi v
Laboratoriju za vzporedno in porazdeljeno računanje na Institutu ``Jožef Stefan'' in sodelujemo v
mednarodnem projektu skupaj z Univerzo v Gentu in Univerzo v Luksemburgu na področju obrabe
materialov.

Tekom naloge si bomo v razdelku~\ref{sec:mehanika} ogledali natančno matematično teorijo linearne
...
elastostatičnega kontaktnega problema iz eksperimenta za raziskovanja obrabe materialov, ki se
ujemajo z nedavno objavljenimi-QUOTE, so opisani v razdelku~\ref{sec:fwo}. V
...
tako strogih predpostav na $\vf$ in $\vt$.
...
zapleten, ko ne moremo direktno izračunati $\L b$? Odgovor na to nudi naslednja trditev.
\begin{trditev}
...
\begin{table}[!h]
  ...
  \begin{tabular}{|m{6.2cm}|l|} \hline
    ...
    \tc{\mbox{Inverzne multikvadratične (IMQ)} \\ \ang{Inverse multiquadrics}
...
seštejemo. Primer take matrike pri reševanju dvodimenzionalnega sistema je prikazan kasneje,
na sliki~\ref{fig:hertz-matrix} na strani~\pageref{fig:hertz-matrix}.
...
sliki~\ref{fig:hertz-refined-domain-together}.
...
ni drugače navedeno, so vse meritve narejene na računalniku z eno centralno procesno enoto
\verb|Intel(R) Core(TM) i7-4700MQ CPU @2.40GHz| s štirimi jedri, od katerih vsako podpira dve niti
izvajanja, in \unit[16]{GiB} DDR3 pomnilnika. Vsi programi so bili prevedeni s prevajalnikom
\verb|g++| verzije 7.1.7 in stikali \verb|-std=c++14 -O3 -DNDEBUG| na operacijskem sistemu Manjaro
Linux.
...
prej. V domeno smo razporedili $N = 209903$ diskretizacijskih točk, po ustvarjanju domene pa
smo jo še dodatno izboljšali z algoritmom~\ref{alg:relax} s 50 iteracijami na šestih sosedih z $F_0
= 10^{-3}$ in $\alpha = 3$. Upogib nosilca, obarvan z vrednostjo von Misesove napetosti je
prikazan na sliki~\ref{fig:cantilever-beam-with-holes}.
...
označujejo najverjetnejša mesta za poškodbe.

\begin{figure}[h]
  \centering
  \includegraphics[width=0.8\textwidth]{images/cantilever_beam_with_holes.png}
  \caption{Numerična rešitev upogiba nosilca z nekaj luknjami. Pomiki so zaradi boljše vidnosti
...
Analitične vrednosti napetosti v okolici kontakta so prikazana na sliki~\ref{fig:hertz-analytical}.
...
\]
Na sliki~\ref{fig:hertz-matrix} je prikazana matrika končnega sistema enačb za zelo grobo
diskretizacijo. Vidimo, da ima bločno stukturo, kot opisano v enačbi~\ref{eq:higher-dim-system} v
razdelku~\ref{sec:visje-dimenzije}, le za primer dveh dimenzij.
...
\begin{figure}[!h]
  ...
  problema~\eqref{eq:hertzian-problem} z 9 monomi in pravokotno diskretizacijo z $11 \times 6$
točkami.}
...
\begin{figure}[!h]
  ...
  \caption{Napaka pri različnih gostotah diskretizacij v odvisnosti od velikosti domene.}
  \label{fig:hertz-domain-too-small}
\end{figure}

Vidimo, da na začetku napaka, pozročena z netočnimi robnimi pogoji, pada in ko pade pod vrednost
napake, dobljene zaradi diskretizacije, večanje $H$ ne pomaga več. Pri bojl natančnih
distretizacijah moramo vzeti tudi večji $H$, saj je napaka diskretizacije toliko nižja.
Če torej ugotovimo, da se konvergenca ustavi ali ne nadaljuje več z enakom redom kot prej,
smo najverjetneje zadeli ob napako zaradi končnih robnih pogojev.

Pri veliki domeni je nemogoče enakomerno gosto večati diskretizacijo. Edina smiselna rešitev je
zgostistev mreže le lokalno v okolici kontakta, kjer nas rešitev zanima bolj natančno.
Izberimo $H = \unit[1]{m} \approx 75\,000\, b$ in zgostimo mrežo kot opisano v
razdelku~\ref{sec:goscenje}. Izbrali smo $\ell = 8$ in $f = 0.4$.
...
\begin{figure}[h]
  ...
  \begin{subfigure}[t]{0.45\textwidth}
    ...
    \caption{Sedemnajstkrat zgoščena diskretizacija okoli področja zanimanja.}
  ...
  \end{subfigure}
  \caption{Primer sedemnajstkrat zgoščene domene s pomočjo katere lahko natančneje rešimo
  problem~\eqref{eq:hertzian-problem}.}
  \label{fig:hertz-refined-domain-together}
\end{figure}

Rešimo problem na zgoščeni domeni z uporabo Gausovih funkcij z parametrom oblike $\sigma_b = 350\,
r_c$, Gaussovo utežjo s $\sigma_w = r_c$ in $n = m = 9$. Sitem enačb smo rešili z iterativnim
BiCGSTAB algoritmom z ILUT predpogojevanjem. Uporabili smo parametra $p=100$ in $\tau = 10^{-6}$.
BiCGSTAB algoritem smo iterirali največ 1000-krat ali dokler ni bila ocena napake pod $10^{-15}$.
...
\begin{figure}[h]
  ...
  \caption{Numerična rešitev problema~\ref{eq:hertzian-problem}. Zaradi boljše vidnosti so
  pomiki povečani za faktor $10^5$.}
  \label{fig:hertz-solution}
\end{figure}

\section{Zgled iz mehanike obrabe materialov}
\label{sec:fwo}
Oglejmo si še primer elastostatičnega problema, ki prihaja iz mehanike obrabe materialov.
Obraba materiala zaradi drgnjenja je pojav, ki se pojavi na kontaktni površini med dvema
materialoma, ki se medsebojno gibljeta. Tako gibanje je ponavadi oscilatorno z majhno amplitudo.
Ker se pod takimi pogoji se lahko zmanjša življenjska doba materiala tudi do
\unit[50]{\%}~\cite{jeung2015crack} glede na življenjsko dobo, pri kateri oscilacij ne bi bilo, je
pomebno dobro razumevanje tega pojava, kot tudi vpliva različnih parametrov uporabljenega materiala
na posledice. Eden izmed pogostih eksperimentov za razumevanje tega pojava se izvede tako, da se
ploščat kos materiala, ki ga imenujmo \emph{vzorec} vpne na enem koncu in drug konec pa periodično
raztegujemo z napetostjo z amplitudo $\sax$. Z drugih dveh strani na vzorec
...
sliki~\ref{fig:fwo-shema}. [TODO Gregor foto?]

\begin{figure}[!h]
  \centering
  \includegraphics[width=\iw]{images/placeholder.pdf}
  \caption{Shema eksperimenta za raziskovanje obrabe materialov.}
  \label{fig:fwo-shema}
\end{figure}

Naprednejše simulacije procesa želijo napovedati tudi rast razpoke v materialu, na
katere smer, še posebno v režimu, ko je razpoka majhna, močno vpliva tangentna sila $Q$.
Za kvalitetno numerično simulacijo razpoke torej potrebujemo dobro poznavanje napetosti na površini
kontakta. Z numerično simulacijo simulacijo eksperimenta so se nedavno ukvarjali Pereira in
drugi~\cite{pereira2016convergence} v sklopu mednarodnega sodelovanja med Univerzo v Luxemburgu,
Univerzo v Gentu in Inštitutom ``Jožef Stefan''. Univerza v Luxemburgu bo postavila in izvedla
zgoraj opisani eksperiment, Univerza v Gentu bo izvedla numerično simulacijo eksperimenta širjenja
razpok z uporabo metode končnih elementov, Inštitut ``Jožef Stefan'' pa bo simuliral eksperiment z
uporabo brezmrežnih metod in poskrbel za paralelizacijo. [TODO Gregor revise?]

Prvi korak pri simulaciji eksperimenta je kvalitetna aproksimacija napetosti na površini kontakta,
ki se mora ujemati z rezultati v~\cite{pereira2016convergence}. Avtorji so uporabili programski
paket ABAQUS\textregistered~\cite{hibbitt2001abaqus} za analizo problema z
uporabo metode končnih elementov, pri čemer so simulirali eksperiment v statičnem stanju maksimalne
oscilacije s tangentno silo enako $Q$ in osno napetostjo enako $\sax$. Gravitacijo v tem
primeru ignoriramo, saj so napetosti zaradi nje mnogo manjše kot napetosti v eksperimentu.
Posledično ima eksperiment vodoravno simetrijsko os in lahko simuliramo eksperiment samo z eno glavo
in polovično debelino, ter ga nato prezrcalimo. Poleg tega se problem ne spreminja vzdolž tretje
dimenzije in ni tanek, torej ustreza predpostavki o ravninski deformaciji in ga lahko prevedemo na dve
dimenziji. Tako so naredili tudi avtorji v~\cite{pereira2016convergence} in diskretizirali glavo in
polovico vzorca ter aproksimirali kontaktne površine in napetosti. Lotimo se enakega problema, a z
drugega zornega kota. Problem nas spominja na teorijo Hertzevega kontakta, na kratko opisano
v~\ref{sec:hertz}, le da ima prisotno tudi tangentno silo, osno napetost in končne dimenzije.
V~\cite[razdelek 2]{pereira2016convergence} so opisani analitični izrazi za modeliranje napetosti, s
katerimi cilindrična glava pritiska na vzorec. Označimo krivinski radij glave z $R$, višino oz.\ debelino
vzorca s $t$ in dolžino z $L$. Če najprej Hertzevemu problemu dodamo končno višino,
se širina polovice kontaktnega območja izraža z
\[
...
\begin{align}
  ...
  [-L/2, L/2]\times[-t/2, 0] \nonumber  \\
  \vt(x, 0) &= q(x)\vi - p(x)\vj, && \text{(napetost na vrhu)} \label{eq:fwo-problem}\\
    \vu(-L/2, y) &= 0, \nonumber  && \text{(vpet levo)}\\
    \vt(L/2, y) &= 0, \nonumber  && \text{(prost desno)}\\
    \vu(x, -t/2)\cdot \vj &= 0, \dpar{\vu(x, -t/2)\cdot \vi}{y} = 0, \nonumber  && \text{(simetrija
    spodaj)}
\end{align}
...
\begin{figure}[!h]
  \centering
  \begin{subfigure}[t]{0.45\textwidth}
    \includegraphics[width=\textwidth]{images/placeholder.pdf}
    \caption{Primer napetosti $p(x)$ in $q(x)$, kot fig. 3 v FWO članku.}
    \label{fig:fwo-pq-example}
  \end{subfigure}
  \begin{subfigure}[t]{0.45\textwidth}
    \includegraphics[width=\textwidth]{images/placeholder.pdf}
    \caption{Domena in robni pogoji. Označili smo $\vu = (u, v)$, $\vt = (t_x, t_y)$.}
    \label{fig:fwo-bc}
  \end{subfigure}
...
\unit[4]{mm}$ in $L = \unit[40]{mm}$. Največji izmerjeni sili sta bili $\vec{F} = -\unit[543]{N}\vj$ in
$\vec{Q} = -\unit[155]{N}\vi$, osna napetost pa je bila $\sax = \unit[100]{MPa}$. Numerična simulacija
v~\cite{pereira2016convergence} je bila narejena za $R = \unit[10]{mm}$ in $R = \unit[50]{mm}\}$ in
tri različne koeficiente trenja $\mu = 0.3$, $\mu = 0.85$ in $\mu = 2$, zato tudi mi izberimo te
parametre. Na Hertzevem kontaktnem problemu~\eqref{eq:hertzian-problem} smo se naučili, kako dobro
diskretizirati domeno. Tudi tukaj uporabimo enake parametre, le goščenje domene prilagodimo
manjšim dimenzijam. Metoda je za vse izbire parametrov $R$ in $\mu$ s povečevanjem $N$ konvergirala
proti limitni rešitvi. Na sliki~\ref{fig:fwo-ujemanje} je prikazano ujemanje naše numerične rešitve
z rešitvijo izračunano v~\cite{pereira2016convergence} za $N = TODO$.

\begin{figure}[h]
  \centering
  \includegraphics[width=\textwidth]{images/fwo_cases.pdf}
  \caption{Ujemanje MSLM in FEM rešitve pri analizi kontakta.}
...
\begin{table}[h]
  ...
  \begin{tabular}{|c|c|c|} \hline
    ...
    re & zul & tatov \\ \hline
  \end{tabular}
  \caption{Tabela vrednosti $\ts_{xx,\max}$ za FEM, MLSM in analitično aproksimacijo.}
  \label{tab:sxxmax}
\end{table}
Vidimo, da vrednosti konvergirajo k neki vrednosti, ki pa se razlikuje pri obeh metodah, vendar je v
istem velikostnem razredu, kar je tudi pričakovati. Primer rešitve za ?? je prikazan na
sliki~\ref{fig:fwo-solution}.

\begin{figure}[h]
  \centering
  \includegraphics[width=\iw]{images/placeholder.pdf}
  \caption{Primer rešitve problema~\eqref{eq:fwo-problem} za $\mu=??$ in $R = ??$. Pomiki so
  povečani za faktor ??, rešitev pa je obarvana z vrednostjo von Misesove napetosti.}
...
računanje.\footnote{\url{http://www-e6.ijs.si/ParallelAndDistributedSystems/}} [TODO kaj o
avtorstvu?] Implementirana je bila v programskem jeziku \CC~\cite{stroustrup1995c++} zaradi hitrosti
in dovolj močnih jezikovnih konstruktov, kot so recimo predloge \ang{templates}, ki omogočajo
elegantno implementacijo.  Podoben koncept uporablja knjižnica
Eigen~\cite{eigenweb}, uporabljena za numerično linearno algebro.
Za iskanje najbližjih sosedov je bila uporabljena knjižnica ANN~\cite{ann}.
Celotna implementacija je prosto dostopna na repozitoriju programske
skupine\footnote{\url{http://gitlab.com/e62Lab/e62numcodes}} z dokumentacijo, prav tako na voljo na
...
\begin{figure}[h]
  \centering
  \includegraphics[width=\iw]{images/placeholder.pdf}
  \caption{Diagram implementacije in izvedbe algoritma~\ref{alg:metoda}.}
...
niti izvajanja, in \unit[64]{GiB} pomnilnika.  Programi so bili prevedeni z Intelovim c++
...
\begin{figure}[h!]
  ...
  \caption{Čas izvajanja posameznih delov programa pri reševanju
  problema~\eqref{eq:poisson-problem}.}
...
\begin{figure}[!h]
  ...
  \caption{Faktorji pohitritve $f_k(N)$ pri reševanju problema~\eqref{eq:poisson-problem}.}
...
Tekom magistrske naloge smo si ogledali klasično teorijo elastomehanike, ki je še vedno aktualna,
saj nudi osnovo probleme z drugih področij, kot na primer kontaktne mehanike. Poleg tega smo si
ogledali splošno formulacijo brezmrežne MLSM metode in pokazali da posplošuje druge znane metode kot
so metoda končnih diferenc in metoda radialnih baznih funkcij. Analizirali smo uporabo različnih
baznih funkcij in uteži, različnega števila sosedov. Pokazali smo, da se s primernimi algoritmi za
določanje položajev diskretizacijskih točk lahko domeno posploši na poljubne domene, z uporabo
izpeljanih algoritmov za goščenje diskretizacije pa se jo lahko uporabi na bolj zapletenih
problemih. Metodo smo testirali na dveh klasičnih primerih iz elastostatike in se naučili, kako jo
najbolje uporabljati. Za konec smo rešili še aktualen primer iz mehanike obrabe materialov in
pokazali ujemanje metode z drugimi metodami ter njeno primernost za reševanje takih problemov.

Nadaljnje delo se lahko giblje v veliko smeri. Metodo samo se lahko obogati z večjo podporo za
...
splošnost, saj ima uporabnik sam na voljo izbiro postopka za reševanje. Na področju implementacije se
