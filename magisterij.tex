...
močni obliki z metodo končnih diferenc~\cite{hattel1994control} in metodo končnih
volumnov~\cite{stone2003dynamic}. Poleg tega so se v bolj bližji preteklosti za reševanje
...
razpok~\cite{rao2000efficient}.  Eno izmed brezmrežnih metod s sodelavci razvijamo tudi v
...
\begin{primer}
  ...
  \begin{figure}[h]
    ...
    \caption[Primer gibanja enotske kocke.]{Prerez enotske kocke pri $Z = 0$ v referenčni konfiguraciji in v prostorski
...
\begin{figure}[ht]
  ...
  \caption[Spremeba kvalitete diskretizacije po izboljšavi.]{Sprememba $h$ in
...
\begin{figure}[h]
  ...
  \begin{subfigure}[t]{0.45\textwidth}
    ...
    \caption[Šetvilo odrezanim singularnih vrednosti.]{Število odrezanih
    singularnih vrednosti v odvisnosti od $\sigma_b$ in $\sigma_w$.}
    \label{fig:poisson-square-sigma-dep-cutoff}
  \end{subfigure}
  \caption[Analiza reševanja Possonove enačbe z Gaussovimi funkcijami in utežmi.]{Analiza reševanja problema~\eqref{eq:poisson-problem} z Gaussovimi funkcijami
...
zgostitev mreže le lokalno v okolici kontakta, kjer nas rešitev zanima bolj natančno.
...
\begin{figure}[h]
  ...
  natančneje rešimo Hertzev kontaktni problem~\eqref{eq:hertzian-problem}.}
...
aproksimacijo in ohranja enak red. 
Numerična rešitev, pridobljena z najfinejšo diskretizacijo je prikazana na
...
\begin{figure}[h]
  ...
  boljše vidnosti so pomiki povečani za faktor $5 \cdot 10^3$.}
  \label{fig:hertz-solution}
\end{figure}

\section{Zgled iz mehanike utrujenosti materialov}
\label{sec:fwo}
Oglejmo si še primer elastostatičnega problema, ki prihaja iz mehanike utrujenosti materialov
\ang{fretting fatigue}.  Obraba materiala zaradi drgnjenja je pojav, ki se pojavi na kontaktni
površini med dvema materialoma, ki se medsebojno gibljeta. Tako gibanje je ponavadi oscilatorno z
majhno amplitudo.  Ker se pod takimi pogoji se lahko zmanjša življenjska doba materiala tudi do
\unit[50]{\%}~\cite{jeung2015crack} glede na življenjsko dobo, pri kateri oscilacij ne bi bilo, je
pomembno dobro razumevanje tega pojava, kot tudi vpliva različnih parametrov uporabljenega materiala
na posledice. Eden izmed pogostih eksperimentov, opisan v~\cite{hojjati2014prediction},  za
razumevanje tega pojava se izvede tako, da se ploščat kos materiala, ki ga imenujmo \emph{vzorec}
vpne na enem koncu in drug konec pa periodično raztegujemo z napetostjo z amplitudo $\sax$. Z drugih
dveh strani na vzorec pritiskata dve cilindrični glavi z normalno silo $F$, ki se tudi gibljeta
oscilatorno tangentno na površino materiala in nanj delujeta s tangentno silo $Q$.  Shema
eksperimenta je prikazana na sliki~\ref{fig:fwo-shema}. [TODO Gregor foto?]
...
lahko prevedemo na dve dimenziji. Tako so naredili tudi avtorji v~\cite{pereira2016convergence} in
diskretizirali glavo in polovico vzorca ter aproksimirali kontaktne površine in napetosti. Lotimo se
enakega problema, a z drugega zornega kota. Problem nas spominja na teorijo Hertzevega kontakta, na
kratko opisano v~\ref{sec:hertz}, le da ima prisotno tudi tangentno silo, osno napetost in končne
...
Če to sliko primerjamo z sliko~\ref{fig:hertz-solution}, na kateri je rešitev Hertzevega kontaktnega
problema opazimo podobno splošno obliko nivojnic napetosti v notranjosti, na površini pa se na
sliki~\ref{fig:fwo-solution} močno pozna vpliv tangente sile $Q$, ki povzroči zamik nivojnic in
...
\begin{figure}[!h]
  ...
  \caption[Faktorji pohitritve pri reševanju Poissonove enačbe.]{Faktorji pohitritve $f_k(N)$ pri reševanju problema~\eqref{eq:poisson-problem}.}
...
najbolje uporabljati. Za konec smo rešili še aktualen primer iz mehanike utrujanja materialov in
