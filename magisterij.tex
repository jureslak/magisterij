...
\begin{definicija}
  Laplaceev operator je na vektorskem polju $\vv$ definiran kot
  \[ \lap \vv = \div\grad \vv.  \]
\end{definicija}
...
\end{izrek}
\proof
Podobno kot pri Cauchyevi recipročni relaciji si izberimo majhno telo okoli
...
\begin{opomba}
  ...
  Cauchyevo momentno enačbo. S tem dobimo enačbo
  \begin{equation}
...
\begin{align}
  ...
  u'(b) &= B, \nonumber
\end{align}
čigar rešitev poznamo v kvadraturah
\[
...
robne pogoje, bomo dobili sistem linearnih enačb, čigar rešitev nam bo dala
...
Izvajanje funkcije \textsc{sosedi} nas stane $O(N)$ prostora za hranjenje drevesa
...
dosežemo natančnost $10^{-6}$ iz grafov~\ref{fig:mlsm-fdm-err}
in~\ref{fig:mlsm-fdm-time} vidimo, da MLSM višjega reda potrebuje le $N = 22$,
...
\]
kar drži, v resnici je najbolj ekstremna vrednost $u(1/2, 1/2) \approx -0.07367$.
...
multikvadratičnimi.  Za monome vzemimo $n = m = 5$, $m = 6$ in $n=9$ in $m=n=9$,
kar sovpada s primeri iz razdelka~\ref{sec:posebni-primeri}. Za radialne bazne
funkcije vzemimo vedno $n=m$ in sicer vrednosti 5, 9 in 13 za parameter oblike
pa $\sigma_b = 110\, r_\chi$. Za utež vzemimo Gaussovo utež z $\sigma_w = \frac34 r_\chi$.
