...
void solve(int n, int nt, T<Vec2d> basis, int support_size, std::string name) {
    ...
    double a = 0, b = 1;
    // Prepare domain
    RectangleDomain<Vec2d> domain(Vec2d({a, a}), Vec2d({b, b}));
...
//    file.setFloatArray("sol", sol);
    ...
    file.setDoubleAttribute("time_total", t.getTime("start", "end"));
    file.setDoubleAttribute("time_domain", t.getTime("start", "domain"));
    file.setDoubleAttribute("time_shapes", t.getTime("domain", "shapes"));
    file.setDoubleAttribute("time_construct", t.getTime("shapes", "construct"));
    file.setDoubleAttribute("time_compute", t.getTime("construct", "compute"));
    file.setDoubleAttribute("time_solve", t.getTime("compute", "end"));
    file.closeFolder();
    file.closeFile();
