...
legendvals = {'G9 -- pomik ($e_\infty(\vec{u})$)',...
    'G9 -- napetost ($e_\infty(\sigma)$)',...
    'MON9 -- pomik ($e_\infty(\vec{u})$)',...
    'MON9 -- napetost ($e_\infty(\sigma)$)'};
...
for i = 1:typenum
    ...
    plot(Ns, data(i, :, 4), [markers{2*i+1}, ':'], 'Color', colors{i}, 'LineWidth', 2)
end
